    ...
    : IRequestHandler<GenerateMealPlanCommand, Guid>
{
    public async Task<Guid> Handle(GenerateMealPlanCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            if (request.PlanType == PlanType.Weekly)
                throw new Exception("Weekly plans are for Premium users only.");

            var generatedTodayCount = await context.MealPlans
                .CountAsync(mp => mp.User.Id == request.UserId && mp.StartDate.Date == today, cancellationToken);
                             
            if (generatedTodayCount >= 1)
                throw new Exception("Free users can only generate 1 plan per day.");
        }
        ...
        var days = request.PlanType == PlanType.Daily ? 1 : 7;
        ...
        foreach (var dayPlan in aiResponse.PlanDays)
        {
            ...
            foreach (var aiMeal in dayPlan.Meals)
            {
                ...
                var ingredient = new Ingredient($"{aiMeal.Name} Base", aiMeal.Calories, aiMeal.Protein, aiMeal.Carbs, aiMeal.Fat);
